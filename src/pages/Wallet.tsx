...
export default function Wallet() {
  ...
  if (authLoading || walletLoading) return <WalletPageSkeleton />;
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 pb-24 space-y-6">
      <div className="bg-gradient-to-br from-primary to-primary/70 rounded-2xl p-6 text-primary-foreground">
        ...
        <p className="text-3xl font-bold mb-4">Rp {(wallet?.balance || 0).toLocaleString("id-ID")}</p>
        <Button variant="secondary" size="sm" onClick={() => setTopUpOpen(true)} disabled={gateways.length === 0} className="gap-1.5">
          <Plus className="w-4 h-4" /> Top Up
        </Button>
        {gateways.length === 0 && <p className="text-xs opacity-70 mt-2">No payment gateway active. Contact admin.</p>}
      </div>
